...
Phases **1–7** are documented below. Post-baseline changelog with eval evidence:
**IMPROVEMENTS_LOG.md**. Forward roadmap: **ROADMAP.md**.

## Live demo

| | URL |
|---|---|
| **App** | https://www.steadyfit.pro |
| **API (Render)** | https://steadyfit-api.onrender.com |
| **Health** | https://steadyfit-api.onrender.com/health → `{"ok":true}` |

Verified working (CORS allows https://www.steadyfit.pro). GitHub: https://github.com/saurabhIU/SteadyFit
...
| Web | **Vercel** (Root Directory = `web`) | https://www.steadyfit.pro |
...
`FRONTEND_URL=https://www.steadyfit.pro` on Render (CORS).
...
## **1**: Defining Problem, Audience, and Scope
...
See **Architecture** above (ASCII overview + mermaid). Hybrid retriever SQL param-order bug and re-run comparison are noted there.
...
| Evaluation | RAGAS + LLM-as-judge; **120** golden cases (**5**/**6** labeled runs were **80**; latest near-full suite **96**) | RAGAS for retrieval quality; LLM-as-judge for tone/safety/plan sanity; category suites for photo_meal, council_critique, diet_plan, calendar_day, try_profile_ux. |
...
10. **Council critique-and-revise**, **photo meal logging**, **try-it-yourself** profiles,
    ...
    **diff-based plan_changed replies**, **condition-aware meal nudges**
    (see **7** shipped + `IMPROVEMENTS_LOG.md`).
...
- **5** labeled baseline was run on the then-current **80-case** set
  (`baseline_fixed`). That historical number is kept below for before/after **6**.
...
**Dataset (**5** snapshot):** 80 cases across 13 categories. Two runs performed:
...
score clean. Corrected in the dataset for **6**.
...
chunking strategy is sound. This motivates the **6** upgrade to hybrid
...
**RAGAS: what improved and what still needs **6****
...
motivating the **6** hybrid retrieval upgrade. Personal document retrieval is
...
`--label hybrid_retrieval` after **6**, then
...
### Shipped since **6** (with eval evidence)
...
| **Live deploy** | Vercel app + Render API with CORS wired | https://www.steadyfit.pro · https://steadyfit-api.onrender.com/health |
...
- **Eval table** — **5**/**6** hybrid delta **plus** 96-case suite / 120-case golden set.
...
- [x] README phases **1–7** include eval numbers (baseline_fixed +
      hybrid_retrieval on 80; current golden **120**; latest suite **96**)
- [x] Architecture docs (README + IMPROVEMENTS_LOG) aligned
- [x] Live URLs verified — https://www.steadyfit.pro ·
